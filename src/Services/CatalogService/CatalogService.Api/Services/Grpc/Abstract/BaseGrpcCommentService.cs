...
public abstract class BaseGrpcCommentService : CommentProtoService.CommentProtoServiceBase, IBaseGrpcService
{
    private IHttpContextAccessor _httpContextAccessor;

    protected BaseGrpcCommentService(IHttpContextAccessor httpContextAccessor)
    {
        _httpContextAccessor = httpContextAccessor;

        this.Language = this.GetAcceptLanguage(_httpContextAccessor);
        this.ProjectName = System.Reflection.Assembly.GetEntryAssembly()?.GetName()?.Name ?? nameof(BaseGrpcCommentService);
