...
namespace Localization.BackgroundTasks.Services.Cache.Abstract;
...
public class RedisService : IRedisService, IDisposable
{
    ...
    public RedisKey[] GetKeys(string prefix, int databaseId = 1)
    {
        ...
        var keys = GetServer().Keys(database: databaseId, pattern: prefix + "*").ToArray();
    ...
    {
        return await _client.GetDatabase(databaseId).CacheOrGetAsync(key, duration, filter);
    }
    #endregion
    ...
    {
        return await _client.GetDatabase(databaseId).CacheOrGetCompressedAsync(key, duration, filter);
    }
    #endregion
    #region Set
